/*
 * Original source code by Joakim(https://github.com/snokvist)
 *
 * YAML Configurator
 *
 ...
 *
 * Example usage:
 *    ./yaml-cli-multi -i config.yaml
 *    ./yaml-cli-multi -i config.yaml -g network.wifi.ssid
 *    ./yaml-cli-multi -i config.yaml -s credentials.password newsecret
 *    ./yaml-cli-multi -i config.yaml -S list.key "[1,2,3,4]"   (dash notation for lists)
 *    ./yaml-cli-multi -i config.yaml -d network.ethernet
 */
